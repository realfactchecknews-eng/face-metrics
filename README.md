...
**Стабильность оценок:** в `worker.js` заданы `temperature: 0.2`, `top_p: 0.9` и
фиксированный `seed: 1337` — одно и то же фото оценивается почти одинаково
(раньше без этих параметров балл «прыгал»). Промпт требует **дробные баллы**
(один знак после запятой). Симметрия в баллах привязана к измеренному `%`.
