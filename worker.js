...
// Достаём баллы из отчёта: общий PSL («ОБЩИЙ_БАЛЛ: 4.6/8», иначе из строки редкости)
// и восемь категорий «СИММЕТРИЯ: 6.5/10».
function parseScores(text) {
  const num = (label, max = '10') => {
    const re = new RegExp(label + '\\s*:\\s*([0-9]+(?:[.,][0-9])?)\\s*/\\s*' + max, 'i');
  ...
  // Общий балл замера — «ОБЩИЙ_БАЛЛ: X/8» модели, он с десятыми (см. normalizeMeasureText).
  // Строка редкости — запасной путь, если «/8» не пришло.
  const said = num('ОБЩИЙ_БАЛЛ', '8');
  const overall = said !== null ? Math.max(0, Math.min(PSL_MAX, said)) : scoreFromRarity(text);
  return { overall, cats, quality, same };
}
...
/* PSL 0-8 в замерах гайда — тот же расчёт, что в обычном разборе (app.js), чтобы
   балл в гайде и в анализе был одним и тем же числом. Лестница должна совпадать с
   RARITY_LADDER в app.js слово в слово — это держит test_measurepsl.mjs.
   Модель пишет «РЕДКОСТЬ: ЛУЧШЕ/ХУЖЕ 1 из N», это z-оценка, PSL = 4 + z (форум
...
4. Разница в съёмке (свет, ракурс, дистанция, качество камеры, бьюти-режим) даёт сдвиг
   до 1.5 балла в категориях и до 1 балла PSL на ровном месте. Всегда проверяй эту гипотезу ПЕРВОЙ, прежде чем
...
ПРО ВОЛОСЫ - ОНИ ВТОРИЧНЫ.
Главное в замере - лицо. Хорошая стрижка по меркам looksmax.org даёт максимум около полбалла PSL
и не спасает слабую структуру, а человек пришёл за лицом, а не за причёской. Поэтому:
- о стрижке и причёске - не больше ОДНОГО пункта, и только под меткой ГРУМИНГ_STYLE;
- в МИДФЕЙС_MAXILLA и других категориях стрижку не советуй, давай управляемую часть самого лица;
- совет по волосам давай, только если на фото видно, что они работают против лица: волосы
  неухожены; стрижка скрывает сильную черту или подчёркивает слабую; заметны залысины или
  редеющие волосы;
- если нынешняя причёска лицу подходит - так и напиши, пусть оставит. Новую стрижку не выдумывай,
  а пункт ГРУМИНГ_STYLE отдай бровям, бороде или уходу;
- не советуй кроп (French crop, текстурный кроп) по умолчанию. Только если на фото есть причина:
  заметные залысины, которые надо скрыть, или круглое либо квадратное лицо, которому нужен
  объём сверху при коротких висках;
- длинные или средние волосы, которые идут лицу, - нормальный выбор, а не проблема;
- если советуешь стрижку, назови причину из этого лица (высота лба, линия роста волос,
  длина лица) и что именно она изменит. Одно название стрижки - это не совет.
...
- МИДФЕЙС_MAXILLA: не меняется, но восприятие меняют снижение жира и отёка, ракурс и свет;
...
прямо скажи переснять - иначе все последующие сравнения будут испорчены.`;
  ...
  const strip = (t) => {
    ...
    out = out.replace(/^\s*РЕДКОСТЬ\s*:.*$/gmi, '');
    out = out.replace(/^\s*[А-ЯЁA-Z_]+\s*:\s*[0-9]+(?:[.,][0-9])?\s*\/\s*(?:10|8)\s*$/gmi, '');
    out = out.replace(/\b[0-9]+(?:[.,][0-9])?\s*\/\s*(?:10|8)\b/g, '[балл скрыт]');
    return out.replace(/\n{3,}/g, '\n\n').trim();
  };
