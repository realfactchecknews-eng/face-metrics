...
function parseAIReport(text) {
  ...
  var computed = computeOverall(byKey);
  if (computed !== null) {
    result.overallSaid = result.overall;   // что назвала модель — для сверки в консоли
...
function renderAIReport(text, skipSideEffects, isTeaser) {
  ...
  if (parsed.overall !== null) {
    console.log("[balance]", "показываем", parsed.overall,
      "· модель сказала", parsed.overallSaid,
