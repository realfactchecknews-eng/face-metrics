...
function computeFaceMetrics(lm) {
  ...
  // Поворот головы в сторону. Наклон мы теперь компенсируем полностью, а поворот
  // компенсировать нельзя: у повёрнутого лица дальняя половина честно короче.
  // Поэтому просто измеряем угол и сообщаем его — и в отчёт, и модели, чтобы она
  // знала, насколько скидывать с оценки симметрии. Для симметричной пары точек
  // после поворота на угол T: разница по z равна 2x*sin(T), разница по x равна
  // 2x*cos(T), откуда tg(T) — их отношение.
  var cL = lm[LM_CHEEK_L], cR = lm[LM_CHEEK_R];
  var yawDeg = 0;
  if (cL && cR && typeof cL.z === 'number' && typeof cR.z === 'number') {
    var dxc = Math.abs(cR.x - cL.x);
    if (dxc > 0) yawDeg = Math.abs(Math.atan2(cL.z - cR.z, dxc) * 180 / Math.PI);
  }

  return { cheekboneWidth: cheekboneWidth, jawWidth: jawWidth, foreheadWidth: foreheadWidth, faceHeight: faceHeight, widthHeightRatio: widthHeightRatio, symmetryScore: symmetryScore, yawDeg: yawDeg };
}
...
//   'plain'  — направление зависит от пола, оценку не даём (см. ниже)
//
// Сужение скул к челюсти намеренно оставлено без вердикта: широкая челюсть
// читается как мужская черта, сильное сужение — как женская, а пол приложение
// не спрашивает. Показываем число и словесное описание без «хорошо/плохо».
var PROP_DEFS = [
  ['lenWidth',  'Длина лица к ширине',   'Face length to width',   1.337, 'mid'],
  ['noseFace',  'Длина носа к лицу',     'Nose to face length',    0.312, 'lower'],
  ['eyeGap',    'Расстояние между глаз', 'Intercanthal ratio',     1.468, 'lower'],
  ['lipsFace',  'Ширина губ к лицу',     'Mouth to face width',    0.371, 'higher'],
  ['foreCheek', 'Ширина лба к скулам',   'Forehead to cheekbones', 0.974, 'lower'],
...
function computeProportions(lm) {
  ...
  var ru = lang() === 'ru';
  // Порог заметности: 6% от среднего. Всё, что ближе, считаем обычным.
  var NEAR = 0.06;
  var WORD = {
  ...
  return PROP_DEFS.map(function (d) {
    ...
    var level;
    if (dir === 'plain') {
      // Без оценки: только словами, насколько сильно лицо сужается к челюсти.
      var word = Math.abs(rel) < NEAR ? (ru ? 'Умеренное' : 'Moderate')
               : rel > 0              ? (ru ? 'Выраженное' : 'Pronounced')
                                      : (ru ? 'Слабое'     : 'Slight');
      return { label: ru ? d[1] : d[2], value: v, verdict: word, level: 'plain' };
    }
    ...
    return { label: ru ? d[1] : d[2], value: v, verdict: WORD[level], level: level };
  });
}
